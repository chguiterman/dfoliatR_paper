...
The `dfoliatR` package provides dendroecologists with tools to infer, quantify, analyze, and visualize tree-ring growth suppression events and to reconstruct forest insect defoliator outbreak regimes.  It is built on the long-accepted host to nonhost comparison methodology used in the 1980s FORTRAN program OUTBREAK [@Swetnam1985; @Swetnam1989; @Swetnam1993]. Key benefits are that `dfoliatR` provides easier control of suppression thresholds, provides additional output tables and high-quality graphics, defines defoliation events more consistently, and operates in the open-source `R` environment. 
...
include_graphics("../paper_elsevier/Output/plot-sites-df-obr.pdf")
```